...
/**
 * Bounded, session-only Gmail scan for the first Android human-test candidate.
 *
 * Privacy boundary:
 * - bearer stays native and is supplied only by MainActivity;
 * - metadata is inspected before FULL retrieval;
 * - FULL is fetched only for exact known-bank transaction signatures;
 * - raw headers/bodies/message ids are never returned to Flutter or persisted;
 * - only derived event fields and aggregate counters cross the MethodChannel.
 *
 * This is intentionally not the production historical sync engine.
 */
object FinancialMailScanner {
    ...
    private data class DerivedEvent(
        ...
        fun toMap(): Map<String, Any?> = mapOf(
            ...
            "evidenceClass" to evidenceClass,
        ...
        while (inspected < MAX_MESSAGES) {
            ...
            for (i in 0 until messages.length()) {
                ...
                val metadata = getJson(
                    "$GMAIL_MESSAGES/${encode(id)}?format=metadata" +
                ...
                candidates += 1
                val full = getJson("$GMAIL_MESSAGES/${encode(id)}?format=full", token)
                fullFetched += 1
                val body = bodyText(full.optJSONObject("payload"))
                val event = extract(candidate, body, headers["date"].orEmpty())
                if (event == null) {
                    parseMisses += 1
                } else {
                    events += event
    ...
    private fun classify(from: String, subject: String): Candidate? {
        ...
        val s = normalize(subject).lowercase(Locale.ROOT)

        if (domain == "notificacionesbcp.com.pe") {
            return when {
                s.contains("realizaste un consumo con tu tarjeta") -> Candidate("BCP_CARD_PURCHASE", "BANK_NOTIFICATION")
                s.contains("realizaste un retiro en un cajero automatico") -> Candidate("BCP_ATM_WITHDRAWAL", "BANK_NOTIFICATION")
                s.contains("constancia de transferencia entre mis cuentas") -> Candidate("BCP_INTERNAL_TRANSFER", "BANK_NOTIFICATION")
                s.contains("constancia de transferencia a otros bancos") -> Candidate("BCP_EXTERNAL_TRANSFER", "BANK_NOTIFICATION")
                s.contains("constancia de pago de tarjeta de credito propia") -> Candidate("BCP_CARD_PAYMENT", "BANK_NOTIFICATION")
                s.contains("constancia de pago de servicio") -> Candidate("BCP_SERVICE_PAYMENT", "BANK_NOTIFICATION")
                else -> null
            }
        }
        ...
        if (domain == "netinterbank.com.pe") {
            return when {
                s.contains("realizaste un consumo con tu tarjeta interbank") -> Candidate("INTERBANK_CARD_PURCHASE", "BANK_NOTIFICATION")
                s.contains("constancia de pago plin") -> Candidate("INTERBANK_PLIN_PAYMENT", "PAYMENT_NOTIFICATION")
                s == "constancia de transferencia" -> Candidate("INTERBANK_TRANSFER", "BANK_NOTIFICATION")
                s == "constancia de pago" -> Candidate("INTERBANK_SERVICE_PAYMENT", "BANK_NOTIFICATION")
                else -> null
            }
        }

        if (domain == "notificaciones.bancoripley.com.pe" && s.contains("pago tarjeta ripley exitoso")) {
            return Candidate("RIPLEY_CARD_PAYMENT", "BANK_NOTIFICATION")
        }

        return null
    }

    private fun extract(candidate: Candidate, rawBody: String, occurredAt: String): DerivedEvent? {
        val body = htmlToText(rawBody)
        val parsed = when (candidate.adapterId) {
            "BCP_CARD_PURCHASE" -> bcpPurchase(body)
            "BCP_ATM_WITHDRAWAL" -> simpleOut(body, listOf("total retirado", "retiro de"), "Cajero automático", "UNKNOWN")
            ...
            "BCP_SERVICE_PAYMENT", "INTERBANK_SERVICE_PAYMENT" -> simpleOut(body, listOf("total pagado", "monto pagado", "monto", "importe", "recibo"), field(body, listOf("empresa", "comercio", "establecimiento")) ?: "Pago de servicio", "EXPENSE")
    ...
    private fun moneyAfter(text: String, labels: List<String>): Money? {
        for (label in labels) {
            ...
            val parsed = money(match.groupValues[1], match.groupValues[2])
    ...
    private fun firstMoney(text: String): Money? {
        val match = Regex("\\b(PEN|USD|US\\$|S\\/.?|\\$)\\s*([0-9][0-9.,\\u00A0 ]*)", RegexOption.IGNORE_CASE).find(text)
            ?: return null
    ...
    private fun money(marker: String, raw: String): Money? {
        ...
        if (lastDot >= 0 && lastComma >= 0) {
            ...
            if (index >= 0) {
                val trailing = token.length - index - 1
                if (trailing == 1 || trailing == 2) decimal = token[index]
            }
        }
    ...
    private fun headers(message: JSONObject): Map<String, String> {
        ...
        for (i in 0 until array.length()) {
            val header = array.optJSONObject(i) ?: continue
            val name = header.optString("name").lowercase(Locale.ROOT)
            if (name == "from" || name == "subject" || name == "date") {
                result[name] = header.optString("value")
            }
        }
        return result
    }
    ...
    private fun getJson(url: String, token: String): JSONObject {
        ...
        try {
            ...
            if (status !in 200..299) throw ScanException("GMAIL_SCAN_HTTP_$status")
