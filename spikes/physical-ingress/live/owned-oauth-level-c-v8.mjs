...
const evidence = {
  ...
  network: {
    measurement: 'HTTP_BODY_BYTES_AND_FULL_RESPONSE_ELAPSED_MS',
    requestByteDefinition: 'request body only; excludes URL, headers and TLS framing',
    responseByteDefinition: 'raw HTTP response body bytes from Response.clone().arrayBuffer()',
...
function classifyEndpoint(input, init = {}) {
  ...
  if (url.hostname === 'gmail.googleapis.com') {
    ...
    if (url.pathname.includes('/messages/')) return url.searchParams.get('format') === 'FULL' ? 'full' : 'metadata';
  }
  return 'other';
}
...
function recordNetwork(category, requestBytes, responseBytes, elapsedMs, status) {
  const item = evidence.network.endpoints[category] ?? evidence.network.endpoints.other;
  const elapsed = roundMs(elapsedMs);
  item.count += 1;
  item.requestBodyBytes += requestBytes;
  item.responseBodyBytes += responseBytes;
  item.totalLatencyMs = roundMs(item.totalLatencyMs + elapsed);
  item.minLatencyMs = item.minLatencyMs === null ? elapsed : Math.min(item.minLatencyMs, elapsed);
  item.maxLatencyMs = Math.max(item.maxLatencyMs, elapsed);
...
async function exchangeAuthorizationCode(code) {
  ...
  const response = await instrumentedFetch(request.url, { method: request.method, headers: request.headers, body: request.body });
...
async function establishMessageHistoryAnchor() {
  ...
  const found = await gmail.listMessages({
  ...
  for (const item of found.slice(0, MAX_ANCHOR_WINDOW_MESSAGES)) {
    ...
    const metadata = await gmail.getMessage({
      id: item.id,
      format: 'METADATA',
...
async function runProbe() {
  ...
  const incremental = await gmail.listHistory({
  ...
  for (const id of ids) {
    evidence.requests.metadata += 1;
    const metadata = await gmail.getMessage({ id, format: 'METADATA', metadataHeaders: ['From', 'Date', 'Subject'] });
  ...
  const full = await gmail.getMessage({ id: selectedId, format: 'FULL' });
  ...
  evidence.requests.history += 1;
  const replay = await gmail.listHistory({
    startHistoryId: baselineHistoryId,
    maxResults: MAX_MESSAGES,
    historyTypes: ['messageAdded']
  });
  ...
  evidence.result = evidence.gmail.productionMetadataGate === 'PASS' &&
    ...
    ? 'READY_FOR_REFRESH_AND_REVOCATION'
    : 'PROBE_PARTIAL';
  await persist();
  return evidence.result;
}
...
async function refreshAndVerifyBeforeRevoke() {
  if (!refreshToken) throw new Error('NO_REFRESH_AUTHORITY');
  if (!coreProbePassed()) throw new Error('CORE_PROBE_NOT_READY_FOR_REFRESH');

  evidence.requests.tokenRefresh += 1;
  const refreshResponse = await instrumentedFetch('https://oauth2.googleapis.com/token', {
    method: 'POST',
  ...
  if (!refreshResponse.ok) {
    evidence.oauth.successfulRefreshBeforeRevoke = `HTTP_${refreshResponse.status}`;
    await persist();
    throw new Error(`PRE_REVOKE_REFRESH_HTTP_${refreshResponse.status}`);
  }
  const payload = JSON.parse(raw);
  if (!payload?.access_token) {
    evidence.oauth.successfulRefreshBeforeRevoke = 'MISSING_ACCESS_TOKEN';
    await persist();
    throw new Error('PRE_REVOKE_REFRESH_MISSING_ACCESS_TOKEN');
  }
  ...
  const refreshedProfile = await refreshedProvider.getProfile();
  evidence.oauth.refreshedBearerGmailUse = refreshedProfile.emailAddress && refreshedProfile.emailAddress === authorizedMailbox ? 'PASS' : 'FAIL';
  if (evidence.oauth.refreshedBearerGmailUse !== 'PASS') {
    await persist();
    throw new Error('REFRESHED_BEARER_GMAIL_USE_FAILED');
  }
  await persist();
}

function networkEvidencePassed() {
  const endpoint = evidence.network.endpoints;
  return evidence.network.totalObservedRequests > 0 &&
    endpoint.tokenExchange.count === 1 &&
    endpoint.tokenRefresh.count >= 2 &&
    endpoint.revoke.count === 1 &&
    endpoint.profile.count >= 2 &&
    endpoint.list.count >= 1 &&
    endpoint.metadata.count >= 1 &&
    endpoint.full.count === 1 &&
    endpoint.history.count >= 2 &&
...
async function revokeAndVerify() {
  ...
  if (evidence.oauth.successfulRefreshBeforeRevoke !== 'PASS' || evidence.oauth.refreshedBearerGmailUse !== 'PASS') {
    throw new Error('PRE_REVOKE_REFRESH_NOT_PROVEN');
  }
  ...
  evidence.requests.tokenRefresh += 1;
  const refreshAttempt = await instrumentedFetch('https://oauth2.googleapis.com/token', {
    method: 'POST',
    headers: { 'content-type': 'application/x-www-form-urlencoded' },
    body: new URLSearchParams({
      grant_type: 'refresh_token',
  ...
  evidence.network.complete = networkEvidencePassed() ? 'PASS' : 'FAIL';
  evidence.executionComplete = true;
  evidence.levelCPass = coreProbePassed() &&
    evidence.oauth.successfulRefreshBeforeRevoke === 'PASS' &&
    evidence.oauth.refreshedBearerGmailUse === 'PASS' &&
    evidence.revocation.providerAcceptedRevoke === 'PASS' &&
    evidence.revocation.refreshAuthorityAfterRevoke === 'DENIED' &&
    evidence.network.complete === 'PASS'
    ? 'PASS'
    : 'FAIL';
  evidence.result = evidence.levelCPass === 'PASS' ? 'LEVEL_C_V8_PASS' : 'LEVEL_C_V8_EXECUTION_COMPLETE_WITH_GAPS';
  ...
  server = http.createServer(async (req, res) => {
    ...
    try {
      if (url.pathname === '/' && (url.searchParams.has('code') || url.searchParams.has('error'))) {
        ...
        return sendHtml(res, 200, 'Authorization received', `<p class="ok">OAuth is live. The Gmail profile identifies the authorized mailbox only; its historyId is NOT used as the sync anchor.</p><p>Send this first harmless anchor email to:</p><p><strong>${escapeHtml(authorizedMailbox)}</strong></p><pre>Subject: ${escapeHtml(anchorSubject)}\n\n${escapeHtml(anchorBody)}</pre><p>Wait until the anchor is visibly present in Inbox, then continue. v8 does not depend on Gmail Search indexing.</p><form method="post" action="/anchor?s=${encodeURIComponent(sessionSecret)}"><button>The anchor is visible in Inbox — establish supported sync anchor</button></form>`);
      }

      if (url.pathname === '/') {
        if (!requireSession(url)) return sendHtml(res, 403, 'FinanceSensor Level C', '<p>Invalid local session.</p>');
        return sendHtml(res, 200, 'FinanceSensor — Gmail Level C v8', `<p>v8 preserves the v7 bounded recent-INBOX proof and adds a real successful refresh before revocation plus sanitized payload-byte and per-endpoint latency evidence. No raw HTTP payload, URL query value, Gmail message ID, mailbox address or credential is written to result evidence.</p><p><a class="button" href="/authorize?s=${encodeURIComponent(sessionSecret)}">Authorize FinanceSensor DEV</a></p>`);
      }
      ...
      if (url.pathname === '/probe' && req.method === 'POST') {
        ...
        return sendHtml(res, 200, 'Controlled Gmail probe finished', `<p>Probe state: <strong>${escapeHtml(status)}</strong></p><p>Anchor: ${escapeHtml(evidence.gmail.anchorEstablished)}; path: ${escapeHtml(evidence.gmail.historySelectionPath)}.</p><p>LIST: ${evidence.requests.list}; METADATA: ${evidence.requests.metadata}; FULL: ${evidence.requests.full}; history: ${evidence.requests.history}; profile: ${evidence.requests.profile}.</p><form method="post" action="/refresh-revoke?s=${encodeURIComponent(sessionSecret)}"><button>Prove refresh, revoke, and verify</button></form>`);
      }

      if (url.pathname === '/refresh-revoke' && req.method === 'POST') {
        if (!requireSession(url)) return sendHtml(res, 403, 'FinanceSensor Level C', '<p>Invalid local session.</p>');
        await refreshAndVerifyBeforeRevoke();
        await revokeAndVerify();
        return sendHtml(res, 200, evidence.levelCPass === 'PASS' ? 'Level C v8 PASS' : 'Level C v8 completed with gaps', `<p><strong>${escapeHtml(evidence.result)}</strong></p><p>Successful pre-revoke refresh: ${escapeHtml(evidence.oauth.successfulRefreshBeforeRevoke)}. Refreshed bearer Gmail use: ${escapeHtml(evidence.oauth.refreshedBearerGmailUse)}. Network evidence: ${escapeHtml(evidence.network.complete)}.</p><p>Sanitized aggregate evidence was written to <code>${RESULT_FILE}</code>. Only endpoint classes, counts, body-byte totals, response-byte totals, elapsed timings and HTTP status aggregates are recorded; no URL query values or HTTP payload content are persisted.</p>`);
      }
