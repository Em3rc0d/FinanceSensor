...
let authorizedEmailAddress = null;
...
const evidence = {
  ...
  limits: {
    ...
    profileBeforeHistoryGate: true,
    unfilteredHistoryDiagnosticOnly: true
  },
  oauth: {
  ...
  gmail: {
    ...
    postSendProfileCheck: 'PENDING',
    mailboxHistoryAdvanced: 'PENDING',
    list: 'SKIPPED_BY_DESIGN',
    incrementalHistory: 'PENDING',
    filteredHistoryRecordCount: 0,
    filteredMessageAddedCount: 0,
    unfilteredDiagnosticUsed: false,
    diagnosticHistoryRecordCount: 0,
    diagnosticMessageChangedCount: 0,
    diagnosticMessageAddedCount: 0,
    diagnosticMessageDeletedCount: 0,
    diagnosticLabelAddedCount: 0,
    diagnosticLabelRemovedCount: 0,
...
function cleanSensitiveMemory() { desktopClientSecret = null; accessToken = null; refreshToken = null; authorizedEmailAddress = null; }
...
function copyDiagnostics(prefix, diagnostics) {
  evidence.gmail[`${prefix}HistoryRecordCount`] = Number(diagnostics?.historyRecordCount ?? 0);
  if (prefix === 'diagnostic') {
    evidence.gmail.diagnosticMessageChangedCount = Number(diagnostics?.messageChangedCount ?? 0);
    evidence.gmail.diagnosticMessageAddedCount = Number(diagnostics?.messageAddedCount ?? 0);
    evidence.gmail.diagnosticMessageDeletedCount = Number(diagnostics?.messageDeletedCount ?? 0);
    evidence.gmail.diagnosticLabelAddedCount = Number(diagnostics?.labelAddedCount ?? 0);
    evidence.gmail.diagnosticLabelRemovedCount = Number(diagnostics?.labelRemovedCount ?? 0);
  } else {
    evidence.gmail.filteredMessageAddedCount = Number(diagnostics?.messageAddedCount ?? 0);
  }
}
...
async function runProbe() {
  ...
  const currentProfile = await provider.getProfile();
  evidence.gmail.postSendProfileCheck = 'PASS';
  if (String(currentProfile.historyId) === String(baselineHistoryId)) {
    evidence.gmail.mailboxHistoryAdvanced = 'NO';
    evidence.result = probeAttempts < MAX_PROBE_ATTEMPTS ? 'WAIT_MAILBOX_HISTORY_AND_RETRY_ONCE' : 'PROBE_PARTIAL_NO_MAILBOX_HISTORY_CHANGE';
    await persist();
    return evidence.result;
  }

  evidence.gmail.mailboxHistoryAdvanced = 'PASS';
  ...
  copyDiagnostics('filtered', incremental.diagnostics);
  let ids = uniqueMessageIds(incremental.history);
  evidence.gmail.historySelectionPath = ids.length ? 'FILTERED_MESSAGE_ADDED' : 'FILTERED_EMPTY';

  if (!ids.length) {
    evidence.gmail.unfilteredDiagnosticUsed = true;
    evidence.requests.history += 1;
    const diagnostic = await provider.listHistory({ startHistoryId: baselineHistoryId, maxResults: MAX_MESSAGES, historyTypes: null });
    copyDiagnostics('diagnostic', diagnostic.diagnostics);
    const diagnosticAddedIds = uniqueMessageIds(diagnostic.history);
    if (diagnosticAddedIds.length) {
      ids = diagnosticAddedIds;
      evidence.gmail.historySelectionPath = 'UNFILTERED_MESSAGE_ADDED_DIAGNOSTIC';
    } else {
      evidence.gmail.historySelectionPath = diagnostic.changed.length ? 'HISTORY_CHANGE_WITHOUT_MESSAGE_ADDED' : 'HISTORY_ADVANCED_BUT_NO_RETURNED_RECORDS';
      evidence.result = 'PROBE_PARTIAL_HISTORY_SHAPE_REVIEW';
      await persist();
      return evidence.result;
    }
  }
  ...
  if (!selectedId) {
    ...
    evidence.result = probeAttempts < MAX_PROBE_ATTEMPTS ? 'MESSAGE_ADDED_BUT_MARKER_NOT_FOUND_RETRY_ONCE' : 'PROBE_PARTIAL_MARKER_NOT_FOUND';
    await persist();
    return evidence.result;
  }
  ...
  evidence.result = evidence.gmail.historySelectionPath === 'FILTERED_MESSAGE_ADDED' && evidence.gmail.productionMetadataGate === 'PASS' && evidence.gmail.extraction === 'PASS' && evidence.gmail.replayObserved === 'PASS'
...
function coreProbePassed() {
  return evidence.oauth.realConsent === 'PASS' &&
    ...
    evidence.gmail.mailboxHistoryAdvanced === 'PASS' &&
    evidence.gmail.incrementalHistory === 'PASS' &&
    evidence.gmail.historySelectionPath === 'FILTERED_MESSAGE_ADDED' &&
    evidence.gmail.metadata === 'PASS' &&
    evidence.gmail.syntheticMarkerMatched === 'PASS' &&
  ...
  server = http.createServer(async (req, res) => {
    ...
    try {
      if (url.pathname === '/' && (url.searchParams.has('code') || url.searchParams.has('error'))) {
        ...
        authorizedEmailAddress = profile.emailAddress;
        evidence.gmail.profileHistoryCursor = 'PASS';
        evidence.gmail.authorizedMailboxShownLocally = authorizedEmailAddress ? 'PASS' : 'UNAVAILABLE';
        authorizationComplete = true;
        await persist();
        return sendHtml(res, 200, 'Authorization received', `<p class="ok">OAuth and the Gmail history cursor are live.</p><p>Send exactly one harmless synthetic email to this authorized Gmail inbox:</p><p><strong>${escapeHtml(authorizedEmailAddress || '[profile email unavailable]')}</strong></p><p>Prefer sending it from a different email account so the test represents a clean inbound message.</p><pre>Subject: ${escapeHtml(syntheticSubject)}\n\n${escapeHtml(syntheticBody)}</pre><p>Wait until the message is visibly present in that inbox, then continue. The address and random marker stay only in local process memory and are not written to evidence.</p><form method="post" action="/probe?s=${encodeURIComponent(sessionSecret)}"><button>The message is visible — run bounded probe</button></form>`);
      }
      if (url.pathname === '/') {
        if (!requireSession(url)) return sendHtml(res, 403, 'FinanceSensor Level C', '<p>Invalid local session.</p>');
        return sendHtml(res, 200, 'FinanceSensor — Gmail Level C v5', `<p>Exact authorized mailbox shown locally, profile-before-history gate, no mailbox listing, ≤${MAX_MESSAGES} changed IDs per attempt, ≤${MAX_FULL_MESSAGES} FULL.</p><p><a class="button" href="/authorize?s=${encodeURIComponent(sessionSecret)}">Authorize FinanceSensor DEV</a></p>`);
      }
      ...
      if (url.pathname === '/probe' && req.method === 'POST') {
        ...
        if (status === 'WAIT_MAILBOX_HISTORY_AND_RETRY_ONCE') return sendHtml(res, 200, 'Mailbox history has not advanced yet', `<p class="warn">Gmail profile still reports the authorization-time history cursor. Confirm the synthetic message is visibly present in the exact inbox shown above, wait a few seconds, then use the one allowed retry.</p><form method="post" action="/probe?s=${encodeURIComponent(sessionSecret)}"><button>Retry bounded probe once</button></form>`);
        if (status === 'MESSAGE_ADDED_BUT_MARKER_NOT_FOUND_RETRY_ONCE') return sendHtml(res, 200, 'New Gmail message observed, marker not found yet', `<p class="warn">Gmail history advanced and messageAdded records exist, but the synthetic marker was not among the bounded candidates. Confirm the exact subject and retry once.</p><form method="post" action="/probe?s=${encodeURIComponent(sessionSecret)}"><button>Retry bounded probe once</button></form>`);
        return sendHtml(res, 200, 'Controlled Gmail probe finished', `<p>Probe state: <strong>${escapeHtml(status)}</strong></p><p>Mailbox history advanced: ${escapeHtml(evidence.gmail.mailboxHistoryAdvanced)}; selection path: ${escapeHtml(evidence.gmail.historySelectionPath)}.</p><p>FULL: ${evidence.requests.full}; history: ${evidence.requests.history}; metadata: ${evidence.requests.metadata}; profile: ${evidence.requests.profile}.</p><form method="post" action="/revoke?s=${encodeURIComponent(sessionSecret)}"><button>Revoke FinanceSensor DEV and verify</button></form>`);
      }
      if (url.pathname === '/revoke' && req.method === 'POST') {
        if (!requireSession(url)) return sendHtml(res, 403, 'FinanceSensor Level C', '<p>Invalid local session.</p>');
        await revokeAndVerify();
        return sendHtml(res, 200, evidence.levelCPass === 'PASS' ? 'Level C PASS' : 'Level C completed with gaps', `<p><strong>${escapeHtml(evidence.result)}</strong></p><p>Sanitized aggregate evidence was written to <code>${RESULT_FILE}</code>. No Gmail address, content, message ID, proof marker, token, authorization code, PKCE verifier, client secret or credential path is included.</p>`);
      }
