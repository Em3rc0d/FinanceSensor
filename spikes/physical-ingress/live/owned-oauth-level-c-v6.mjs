...
const evidence = {
  ...
  limits: {
    ...
    maxAnchorListResults: MAX_ANCHOR_LIST_RESULTS,
...
async function establishMessageHistoryAnchor() {
  ...
  evidence.gmail.targetedAnchorSearch = 'PASS';
  ...
  evidence.gmail.anchorMetadata = 'PASS';
  const subject = headerValue(metadata.headers, 'Subject');
  if (subject !== anchorSubject || !metadata.historyId) {
    evidence.gmail.anchorSubjectMatched = subject === anchorSubject ? 'PASS' : 'FAIL';
    evidence.result = 'ANCHOR_METADATA_MISMATCH';
    await persist();
    return evidence.result;
  }

  evidence.gmail.anchorSubjectMatched = 'PASS';
  baselineHistoryId = String(metadata.historyId);
  ...
  server = http.createServer(async (req, res) => {
    ...
    try {
      if (url.pathname === '/' && (url.searchParams.has('code') || url.searchParams.has('error'))) {
        ...
        return sendHtml(res, 200, 'Authorization received', `<p class="ok">OAuth is live. The Gmail profile is used only to identify the authorized mailbox; its historyId is NOT used as the sync anchor.</p><p>Send this first harmless anchor email to:</p><p><strong>${escapeHtml(authorizedMailbox)}</strong></p><pre>Subject: ${escapeHtml(anchorSubject)}\n\n${escapeHtml(anchorBody)}</pre><p>Wait until the anchor is visibly present in that inbox, then continue.</p><form method="post" action="/anchor?s=${encodeURIComponent(sessionSecret)}"><button>The anchor is visible — establish supported sync anchor</button></form>`);
      }
