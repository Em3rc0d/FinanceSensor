...
server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/') {
      ...
      return sendHtml(res, 200, 'FinanceSensor — Gmail Level C v2', `<p>This is a <strong>local-only</strong> controlled OAuth probe. It requests exactly <code>gmail.readonly</code>, stores no token on disk, performs no historical mailbox listing, inspects at most ${MAX_MESSAGES} changed messages per attempt and fetches at most ${MAX_FULL_MESSAGES} FULL message.</p><p><a class="button" href="/authorize?s=${encodeURIComponent(sessionSecret)}">Authorize FinanceSensor DEV</a></p>`);
    }
    ...
    if (url.pathname === '/revoke' && req.method === 'POST') {
      ...
      sendHtml(res, 200, 'Level C finished', `<p class="${status === 'LEVEL_C_EXECUTION_COMPLETE' ? 'ok' : 'warn'}">${escapeHtml(status)}</p><p>Sanitized evidence was written to <code>${RESULT_FILE}</code>. Upload only that JSON.</p><p>You may close this window.</p>`);
    ...
    sendHtml(res, 500, 'FinanceSensor Level C — stopped safely', `<p class="warn">The probe stopped: <code>${escapeHtml(evidence.failureCode)}</code>.</p><p>No secret is shown here. Close the window and share only <code>${RESULT_FILE}</code>.</p>`);
  }
});
...
server.listen(0, HOST, () => {
  ...
  console.log(opened ? 'FinanceSensor Level C v2 opened in your browser.' : `Open this local URL in your browser: ${localUrl}`);
  console.log(`Bounded Gmail policy: no historical list; max ${MAX_MESSAGES} post-authorization changes per attempt; max ${MAX_FULL_MESSAGES} FULL.`);
