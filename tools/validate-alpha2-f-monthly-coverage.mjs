...
assert(receipt?.pullRequest === 75, 'RECEIPT_PR');
assert(receipt?.candidateHeadSha === '6b6a59f5080597dccd0521c96b12050c209a6244', 'RECEIPT_HEAD_SHA');
assert(receipt?.mergeCommitSha === '07fc9e56e0a3b92015a6b2b3eb6e8980de7059a8', 'RECEIPT_MERGE_SHA');
const workflows = new Map((receipt?.workflows ?? []).map(run => [run.name, run]));
for (const [name, runId, runNumber] of [
  ['FinanceSensor Statement ETL Contract', 33998869985, 98],
  ['FinanceSensor Gmail Historical', 33998869969, 138]
]) {
...
const frozenStates = ['OPEN_LIVE','WAITING_FOR_STATEMENTS','IMPORTING','RECONCILING','REVIEW_REQUIRED','RECONCILED','REOPENED'];
...
const coverage = overrides => ({
  id: 'coverage-validator', tenantId: 'tenant-validator', ownerNodeId: 'account-validator',
  periodStart: '2026-09-01T00:00:00.000Z', periodEnd: '2026-09-30T23:59:59.999Z',
  expectedSourceState: ExpectedSourceState.EXPECTED, scopeState: CoverageScopeState.INCLUDED,
  statementState: StatementCoverageState.PARSED, inflowCoverageState: InflowCoverageState.COVERED,
  outflowCoverageState: OutflowCoverageState.COVERED, periodCoverageState: PeriodCoverageState.COVERED,
  reconciliationState: AccountReconciliationState.RECONCILED, requiredDirections: ['INFLOW','OUTFLOW'],
  unresolvedCount: 0, blockingConflictCount: 0, statementPeriodId: 'period-validator', ...overrides
});
const evaluate = coverages => evaluateMonthlyClose({
  tenantId: 'tenant-validator', calendarYear: 2026, calendarMonth: 9,
  closeRequested: true, activity: CloseActivity.IDLE, coverages
});
assertThrows(() => evaluate([coverage({ inflowCoverageState: 'OBSERVED' })]), /MONTHLY_COVERAGE_INFLOW_STATE_INVALID/, 'INFLOW_OBSERVED_MUST_FAIL_CLOSED');
const observedOutflow = evaluate([coverage({ inflowCoverageState: InflowCoverageState.UNKNOWN, outflowCoverageState: OutflowCoverageState.OBSERVED, requiredDirections: ['OUTFLOW'], reconciliationState: AccountReconciliationState.PARTIAL })]);
...
for (const marker of ['getMonthlyCloseReplay','putAccountPeriodCoverage','putMonthlyClose','putMonthlyCloseReplay','LATE_EVIDENCE','PARSER_REPROCESSING','globalUnqualifiedPercentage: null','authoritativeCompletenessPercent: null']) {
  assert(source.includes(marker), `SOURCE_MARKER:${marker}`);
}
for (const forbidden of ['console.log(','writeFileSync(','localStorage','sessionStorage','gmail.googleapis.com','oauth2.googleapis.com','accounts.google.com']) {
  assert(!source.includes(forbidden), `FORBIDDEN_SOURCE:${forbidden}`);
}
assert(/BUILD_READY\s+NO/.test(status), 'GLOBAL_BUILD_READY_STATUS');

console.log('ALPHA2_F_STATIC_IMPLEMENTATION=PASS');
console.log('ALPHA2_F_EXACT_SHA_RECEIPT=PASS');
console.log(`ALPHA2_F_IMPLEMENTATION_HEAD=${receipt.candidateHeadSha}`);
console.log(`ALPHA2_F_MERGE_COMMIT=${receipt.mergeCommitSha}`);
