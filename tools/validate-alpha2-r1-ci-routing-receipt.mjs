...
assert(graph.handoffBundle?.status === 'READY_FROZEN', 'v7 must be frozen');
assert(graph.handoffBundle?.sha256 === bundleSha && graph.handoffBundle?.bytes === bundleBytes, 'v7 frozen bytes drifted');
if (graph.handoffBundle?.generationReceipt !== null) assert(graph.handoffBundle?.generationReceipt?.independentBundleAudit === 'PASS', 'independent v7 generation audit missing');
