...
const bundleSha = '6c5c2baa20f7a266d5ec2b45f225124a0a15e7381776c739f091fead5023cd1d';
const bundleBytes = 86227316;
...
assert(graph.handoffBundle?.status === 'READY_FROZEN', 'v6 must be frozen');
assert(graph.handoffBundle?.sha256 === bundleSha && graph.handoffBundle?.bytes === bundleBytes, 'v6 frozen bytes drifted');
assert(graph.handoffBundle?.generationReceipt?.independentBundleAudit === 'PASS', 'independent v6 generation audit missing');
...
console.log(`R1_V6_BUNDLE_SHA256=${bundleSha}`);
console.log(`R1_V6_BUNDLE_BYTES=${bundleBytes}`);
