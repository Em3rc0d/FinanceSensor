...
assert(physical.status === 'BLOCKED_BY_R1_CURRENT_CANDIDATE', 'current physical state must be blocked by R1');
assert(physical.installPass === false && physical.launchPass === false && physical.oauthPass === false && physical.inheritanceFromPriorCandidateAllowed === false, '+2007 physical evidence cannot be inherited');
assert(/\+2008|stable trusted-edge|R1/i.test(physical.reason ?? ''), 'canonical physical boundary must identify +2008 R1 reacquisition');
...
assert(graph.signing?.status === 'PENDING_CURRENT_CANDIDATE' && graph.signing?.trustedEdgeSigningPass === false, '+2008 signing must be pending');
assert(graph.signing?.signedApkSha256 === null && graph.signing?.signedApkBytes === null && graph.signing?.receipt === null, 'no +2008 stable-signed identity may exist before trusted-edge signing');

for (const key of ['ownedDeviceInstallPass','ownedDeviceLaunchPass','ownedDeviceStableSignerOauthPass','physicalSqlcipherPass','physicalAlpha2Pass','buildReady','releaseReady']) {
  assert(graph.boundaries?.[key] === false, `${key} must remain false`);
}

const consensus = new Map((graph.postMergeConsensus ?? []).map(x => [x.workflow, x]));
for (const [workflow, runId] of [
  ['Alpha.2 Integrated Runtime',34874126273],
  ['FinanceSensor Heartbeat',34874126346],
  ['FinanceSensor Public Readiness',34874126366],
  ['FinanceSensor Mobile Shell',34874126458],
  ['Alpha.2 R2 Owned-Device Campaign Contract',34874126286],
]) {
...
for (const marker of [expected.candidate,expected.sourceCommit,expected.apkSha256,String(expected.apkBytes),String(expected.runId),String(expected.artifactId),expected.signerSha1,'ALPHA2_MOBILE_INTEGRATION_PHYSICAL=OPEN','BUILD_READY=NO','RELEASE_READY=NO']) {
...
console.log('R1_TRUSTED_EDGE_SIGNING=PENDING_CURRENT_CANDIDATE');
