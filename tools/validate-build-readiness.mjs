...
if (!failures.length) {
  ...
  const expectedGateIds = [
    'PRODUCT_DEFINITION', 'MK0_SCOPE', 'P0_QUARRIES', 'TENANCY_MODEL', 'FINANCIAL_MODEL',
    'EVENT_INVARIANTS', 'EDGE_CLOUD_BOUNDARY', 'PRIVACY_MODEL', 'THREAT_MODEL', 'GMAIL_FEASIBILITY',
    'ANDROID_FEASIBILITY', 'MULTI_DEVICE_DESIGN', 'SIGNATURE_WIREFRAMES', 'NO_SCROLL_CONTRACT', 'IMPLEMENTATION_PLAN'
  ];
  ...
  for (const gate of gates) {
    ...
    for (const nodeId of gate.blockingNodes ?? []) if (!ledger.nodes?.find(node => node.id === nodeId)) fail(`${gate.id} references unknown closure node ${nodeId}`);
  }

  for (const id of ['Q-001', 'Q-002']) if (ledger.nodes.find(node => node.id === id)?.status !== 'CLOSED') fail(`${id} must remain CLOSED for financial model PASS`);
  if (byId.get('TENANCY_MODEL')?.state !== 'PASS') fail('TENANCY_MODEL must be PASS after ADR-001 freeze');
  for (const marker of ['TENANT = FINANCIAL OWNERSHIP BOUNDARY','USER != TENANT','DEVICE != TENANT','CONNECTION != TENANT','TENANCY_MODEL_FROZEN != RLS_PHYSICALLY_PROVEN']) if (!tenancyAdr.includes(marker)) fail(`ADR-001 missing tenancy marker: ${marker}`);
  if (byId.get('MK0_SCOPE')?.state !== 'PASS') fail('MK0_SCOPE must be PASS after scope freeze');
  for (const marker of ['MK0_SCOPE = FROZEN','PRIMARY PRODUCT                  MOBILE APPLICATION','FIRST PHYSICAL TARGET           ANDROID','NEW_UNJUSTIFIED_PRODUCT_SURFACE = FORBIDDEN','SCOPE_FROZEN != BUILD_READY']) if (!scope.includes(marker)) fail(`MK0 scope freeze missing marker: ${marker}`);
  if (byId.get('IMPLEMENTATION_PLAN')?.state !== 'PASS') fail('IMPLEMENTATION_PLAN must be PASS after slice freeze');
  for (const marker of ['IMPLEMENTATION_PLAN = PASS','BUILD_SEQUENCE       = B0 → B11','B3 — Gmail mobile OAuth custody','B7 — Control plane and tenant RLS','B11 — Physical closure and gate reconciliation']) if (!implementationPlan.includes(marker)) fail(`implementation plan missing marker: ${marker}`);
  ...
  for (const marker of ['THREAT_MODEL = PASS_FOR_BUILD_ENTRY','SECURITY_PHYSICALLY_PROVEN = NO','SEC_001 = STILL_DRAFTED_UNTIL_Q003_Q004_Q005_CLOSE']) if (!threatModel.includes(marker)) fail(`threat model missing marker: ${marker}`);
  if (byId.get('SIGNATURE_WIREFRAMES')?.state !== 'PASS') fail('SIGNATURE_WIREFRAMES must be PASS for pre-physical build entry');
  for (const marker of ['# S-01 — Home','# S-04 — Financial Sensor','# S-05 — Opportunity','# S-06 — Needs Review','State first','Explainable summary','Sensor signal','One action']) if (!signatureWireframes.includes(marker)) fail(`signature wireframes missing marker: ${marker}`);
  if (byId.get('NO_SCROLL_CONTRACT')?.state !== 'PASS') fail('NO_SCROLL_CONTRACT must be PASS for pre-physical build entry');
  for (const marker of ['VIEW-003','VIEW-004','VIEW-006','VIEW-009','VIEW-010']) if (!viewportContract.includes(marker)) fail(`viewport contract missing marker: ${marker}`);
  for (const marker of ['Size(360, 800)','compact Home is structurally no-scroll','compact Sensor is structurally no-scroll','Opportunity signature fits compact viewport','Needs Review signature fits compact viewport','Movements remains scrollable']) if (!mobileWidgetTests.includes(marker)) fail(`mobile viewport test missing marker: ${marker}`);
  for (const marker of ['PRE_PHYSICAL_BUILD_ENTRY=PASS','PRE_PHYSICAL_DOCUMENT_BLOCKERS=0','PHYSICAL_ANDROID_USABILITY=OPEN','MOBILE_OAUTH_PHYSICAL_PROVEN=NO','SECURITY_PHYSICALLY_PROVEN=NO','WF_001=CLOSURE_OPEN','SEC_001=CLOSURE_OPEN','BUILD_READY=NO','APK_BUILD_PASS != BUILD_READY','SYNTHETIC_WIDGET_PASS != PHYSICAL_ANDROID_PASS']) if (!prePhysicalReceipt.includes(marker)) fail(`pre-physical receipt missing marker: ${marker}`);

  for (const id of ['Q-003', 'Q-004', 'Q-005']) if (ledger.nodes.find(node => node.id === id)?.status !== 'ACTIVE') fail(`${id} must remain ACTIVE until physical closure`);
  if (ledger.nodes.find(node => node.id === 'SEC-001')?.status !== 'DRAFTED') fail('SEC-001 must remain DRAFTED');
  if (ledger.nodes.find(node => node.id === 'WF-001')?.status !== 'DRAFTED') fail('WF-001 must remain DRAFTED');
  if (ledger.buildReady !== false || manifest.buildReady !== false) fail('build readiness must remain false before G-MK0 closure');
  if (ledger.nodes.find(node => node.id === 'G-MK0')?.status === 'CLOSED') fail('G-MK0 unexpectedly CLOSED while buildReady=false');
  ...
  if (new Set(manifest.prePhysicalClosureTargets ?? []).size !== 0) fail('expected 0 pre-physical blockers after Track A closure');
  const physical = new Set(manifest.physicalClosureTargets ?? []);
  for (const id of ['P0_QUARRIES','EDGE_CLOUD_BOUNDARY','PRIVACY_MODEL','GMAIL_FEASIBILITY','ANDROID_FEASIBILITY','MULTI_DEVICE_DESIGN']) if (!physical.has(id)) fail(`physicalClosureTargets missing ${id}`);
...
console.log('NEXT_EXECUTION_NODE=R2_OD3_BOUNDED_FETCH_ONLY_FOR_ALLOWED_PROFILE');
