...
const currentR1Path = 'graph/physical-receipts/ALPHA2-R1-TRUSTED-EDGE-SIGNING-2008-2026-09-14.json';
...
const requiredPaths = [campaignPath,schemaPath,remainderPath,canonicalPath,r1Path,currentR1Path,physicalPath,historicalR1Path,historicalOd3Path,nativeScannerPath,pipelinePath,mainPath];
const failures = [];
...
if (!failures.length) {
  ...
  const currentR1 = readJson(currentR1Path);
  ...
  if (campaign.status !== 'IN_PROGRESS') fail('R2 must be IN_PROGRESS after +2008 R1 signing PASS');
  ...
  if (candidate.minSdk !== 31 || candidate.installabilityObservation !== 'NOT_EXECUTED_STABLE_SIGNED_CURRENT_CANDIDATE' || candidate.stableSignedInstallability !== 'READY_FOR_OD0') fail('current +2008 installability state drifted');
  if (campaign.r1PhysicalReceipt !== currentR1Path || campaign.receipt?.current !== null) fail('R1 receipt must be bound while OD0 receipt remains absent');
  ...
  if (canonical.signing?.trustedEdgeSigningPass !== true || canonical.signing?.signedApkSha256 !== expected.signedApkSha256 || canonical.signing?.signedApkBytes !== expected.signedApkBytes || canonical.signing?.receipt !== currentR1Path) fail('canonical +2008 signing PASS drifted');
  ...
  if (r1.candidate !== expected.candidate || r1.sourceCommit !== expected.sourceCommit || r1.status !== 'TRUSTED_EDGE_SIGNING_PASS' || r1.trustedEdgeSigningPass !== true) fail('R1 +2008 PASS missing');
  if (r1.signedApkSha256 !== expected.signedApkSha256 || r1.signedApkBytes !== expected.signedApkBytes || r1.physicalReceipt?.path !== currentR1Path) fail('R1 +2008 stable-signed binding drifted');
  if (currentR1.signedApkSha256 !== expected.signedApkSha256 || currentR1.signedApkBytes !== expected.signedApkBytes || currentR1.sanitizationPass !== true) fail('current +2008 R1 receipt drifted');
  ...
  if (remainderR1?.status !== 'CLOSED' || remainderR2?.status !== 'DESIGN_FROZEN_EXECUTION_OPEN') fail('prebuild remainder R1/R2 execution state drifted');
  ...
  if (subgates[0]?.status !== 'READY_FOR_PHYSICAL') fail('OD0 must be ready for physical execution');
  ...
  for (const marker of ['private val handles = linkedMapOf<String, NativeHandle>()','if (!candidate.profile.runtimeFetchEnabled)','/attachments/','ALPHA2_STATEMENT_ATTACHMENT_TIMEOUT','ALPHA2_STATEMENT_ATTACHMENT_IO_RETRY_EXHAUSTED','ALPHA2_STATEMENT_ATTACHMENT_RESPONSE_INVALID','ALPHA2_STATEMENT_ATTACHMENT_SIZE_MISMATCH']) if (!nativeScanner.includes(marker)) fail(`native fetch boundary missing marker: ${marker}`);
  for (const marker of ['class Alpha2PipelineStageException','ALPHA2_REFRESH_SCAN_FAILED','ALPHA2_REFRESH_VAULT_READ_FAILED','ALPHA2_REFRESH_PROJECTION_FAILED','Future<void> _safeRelease(String handle)']) if (!pipeline.includes(marker)) fail(`pipeline safe-stop marker missing: ${marker}`);
  ...
  if (state.nextGate !== 'OD0' || state.currentBlocker !== 'OWNED_ANDROID_OD0_INSTALL_AND_LAUNCH_REQUIRED') fail('current next gate/blocker drifted');
...
console.log('R2_CURRENT_BLOCKER=OWNED_ANDROID_OD0_INSTALL_AND_LAUNCH_REQUIRED');
