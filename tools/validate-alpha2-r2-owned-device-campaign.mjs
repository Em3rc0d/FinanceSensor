...
if (!failures.length) {
  ...
  // The prebuild design is an immutable design-freeze snapshot. When source/APK identity changes,
  // its own law reopens R1/R2; current binary identity must then come from the canonical receipt,
  // not from the historical snapshot embedded at the old freeze SHA.
  const reopenLaws = new Set(remainder.executionLaws ?? []);
  if (!reopenLaws.has('SOURCE_OR_APK_IDENTITY_CHANGE_REOPENS_SIGNING_AND_OWNED_DEVICE_CAMPAIGN')) {
    fail('prebuild design missing source/APK reopen law');
  }
  ...
  if (canonical.supersedes?.candidate !== remainder.canonicalAlpha2?.candidate) fail('current canonical receipt must explicitly supersede historical prebuild snapshot');
  ...
  if (canonical.authority?.androidMinSdk !== 24) fail('current canonical minSdk must remain 24');
  if (canonical.authority?.apkSignatureVerify !== 'PASS' || canonical.authority?.apkAapt2Parse !== 'PASS') fail('current canonical APK parser/signature gates must PASS');
  ...
  if (r1.status !== 'HANDOFF_READY_PHYSICAL_OPEN') fail(`R1 must remain HANDOFF_READY_PHYSICAL_OPEN before physical receipt; got ${r1.status}`);
  if (r1.handoffBundle?.status !== 'READY') fail('R1 v4 bundle must be READY before R2 contract is valid');
  ...
  for (const required of frozenR2?.subgates ?? []) {
    if (!subgates.some(x => x.contract === required)) fail(`R2 campaign missing frozen subgate ${required}`);
  }
