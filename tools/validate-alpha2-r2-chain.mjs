...
const od0Pass = r2.receipt?.current === expectedOd0Receipt && r2.subgates?.[0]?.status === 'PASS';
...
if (!signed) {
  ...
  if (r1.physicalReceipt?.path !== expectedR1Receipt || r2.r1PhysicalReceipt !== expectedR1Receipt) throw new Error('R1/R2 receipt chain drifted');
  ...
  if (od0Pass) {
    if (!fs.existsSync(expectedOd0Receipt)) throw new Error('OD0 sanitized physical receipt missing');
    ...
    if (r2.subgates?.[1]?.status !== 'READY_FOR_PHYSICAL') throw new Error('OD1 must be ready after OD0 PASS');
    for (const gate of r2.subgates?.slice(2) ?? []) if (gate.status !== 'BLOCKED_BY_PRIOR_GATE') throw new Error(`${gate.id} must remain blocked by prior gate`);
  } else {
...
console.log(`NEXT_GATE=${od0Pass ? 'OD1_EXACT_GMAIL_READONLY_OAUTH' : signed ? 'OD0_SIGNED_APK_INSTALL_AND_LAUNCH' : 'R1_TRUSTED_EDGE_SIGNING'}`);
