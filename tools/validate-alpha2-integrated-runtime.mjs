...
for (const marker of ['ACCOUNT_MAPPING_REQUIRED','resolveAlpha2StatementOwnership(','confirmAlpha2OwnershipByUser(','Alpha2AccountReconciliationState.reconciled','evaluateAlpha2MonthlyClose(']) {
  if (!productGate.includes(marker)) throw new Error(`ALPHA2_PRODUCT_GATE_MARKER_MISSING:${marker}`);
}
if (!monthlyCoverage.includes('externalBlockingReasons')) throw new Error('ALPHA2_MONTHLY_EXTERNAL_BLOCKS_NOT_MODELED');
if (!monthlyCoverage.includes('externalBlocks.isNotEmpty')) throw new Error('ALPHA2_MONTHLY_EXTERNAL_BLOCKS_NOT_FAIL_CLOSED');
if (!sensor.includes("reason == 'ACCOUNT_MAPPING_REQUIRED'")) throw new Error('ALPHA2_ACCOUNT_MAPPING_SENSOR_GAP_NOT_WIRED');
if (!sensor.includes('monthlyClose.externalBlockingReasons')) throw new Error('ALPHA2_EXTERNAL_PRODUCT_GAPS_NOT_SURFACED');
if (!main.includes('Estado mensual') || !main.includes('Mapeo de cuenta')) throw new Error('ALPHA2_MOBILE_COVERAGE_PRODUCT_STATE_MISSING');
if (!main.includes('Alpha2FinanceInsightsSections(projection: projection)')) throw new Error('ALPHA2_MOBILE_FINANCE_INSIGHTS_NOT_WIRED');
...
for (const marker of ['summarizeAlpha2Categories','Alpha2FlowDirection.outflow','Alpha2SemanticType.cardPayment','summarizeAlpha2KnowledgeGaps']) {
...
for (const key of ['confidence', 'matchScore', 'evidencePercent', 'rawPdf', 'pdfPassword']) {
  if (!projection.includes(`'${key}'`) || !webModel.includes(`'${key}'`)) throw new Error(`ALPHA2_PUBLIC_FORBIDDEN_KEY_NOT_GATED:${key}`);
}
...
if (!webSurface.includes('Entró') || !webSurface.includes('Salió') || !webSurface.includes('Cobertura')) throw new Error('ALPHA2_WEB_CORE_UX_MISSING');
