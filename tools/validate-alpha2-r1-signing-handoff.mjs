...
const expected = {
  ...
  apkBytes: 182092699,
  signedApkSha256: '530ef3fa17c22f94ef0a94aaf625df2ef33022c84d16fad2604a3e0dfc5e0b85',
...
assert(graph.inputApk?.ownedDeviceInstallPass === false && graph.inputApk?.ownedDeviceLaunchPass === false && graph.inputApk?.physicalClaimsInheritedFromPriorCandidate === false, 'OD0 claims must remain open after signing only');
...
assert(graph.handoffBundle?.name === expected.bundleName && graph.handoffBundle?.status === 'READY_FROZEN', 'v7 frozen bundle state drifted');
...
assert(oldV6?.safeToUse === false, 'v6 supersession boundary missing');
...
assert(receipt.candidate === expected.candidate && receipt.sourceCommit === expected.sourceCommit, 'current physical receipt identity drifted');
assert(receipt.canonicalRunId === expected.canonicalRunId && receipt.canonicalArtifactId === expected.canonicalArtifactId, 'current physical receipt canonical authority drifted');
assert(receipt.inputApkSha256 === expected.apkSha256 && receipt.inputApkBytes === expected.apkBytes, 'current receipt input APK drifted');
assert(receipt.signedApkSha256 === expected.signedApkSha256 && receipt.signedApkBytes === expected.signedApkBytes, 'stable signed APK identity drifted');
assert(receipt.signerSha1 === expected.signerSha1 && receipt.androidPackage === 'com.financesensor.lab.gmailconnection.r2' && receipt.gmailScope === 'gmail.readonly', 'receipt signer/package/scope drifted');
assert(receipt.trustedEdgeSigningPass === true && receipt.r2OwnedDeviceCampaignUnblocked === true, 'receipt must close R1 and unblock R2 only');
assert(receipt.sanitizationPass === true && receipt.rawPrivateMaterialCommitted === false, 'receipt sanitization boundary drifted');
assert(receipt.physicalAlpha2Pass === false && receipt.buildReady === false && receipt.releaseReady === false, 'receipt cannot promote downstream readiness');

assert(graph.physicalReceipt?.path === receiptPath && graph.physicalReceipt?.sanitizationPass === true && graph.physicalReceipt?.rawPrivateMaterialCommitted === false, 'current R1 receipt binding drifted');
assert(graph.status === 'TRUSTED_EDGE_SIGNING_PASS' && graph.trustedEdgeSigningPass === true, 'R1 must be closed by +2005 trusted-edge receipt');
assert(graph.signedApkSha256 === expected.signedApkSha256 && graph.signedApkBytes === expected.signedApkBytes, 'R1 stable APK identity drifted');
assert(graph.physicalAlpha2Pass === false && graph.buildReady === false && graph.releaseReady === false, 'R1 signing cannot promote physical/build/release gates');
...
for (const marker of ['node tools/validate-alpha2-canonical-candidate.mjs','node tools/validate-alpha2-r1-signing-handoff.mjs','node tools/validate-alpha2-r1-physical-signing-receipt.mjs','R1_V7_FROZEN_BYTES=PASS','PUBLIC_CI_ORIGINATED_PHYSICAL_PASS=0','PHYSICAL_ALPHA2_PASS=NO','BUILD_READY=NO','RELEASE_READY=NO',expected.bundleSha256,String(expected.bundleBytes)]) assert(workflow.includes(marker), `R1 workflow missing marker: ${marker}`);
