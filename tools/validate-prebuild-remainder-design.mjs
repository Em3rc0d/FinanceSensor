...
if (!failures.length) {
  ...
  // The design explicitly says a source/APK identity change reopens R1/R2. Therefore the
  // current canonical receipt may supersede this snapshot without mutating the design freeze.
  ...
  if (canonical.supersedes?.candidate !== frozen.candidate || canonical.supersedes?.apkSha256 !== frozen.apkSha256) fail('current canonical receipt must bind superseded freeze snapshot');
  ...
  if (canonical.authority?.androidMinSdk !== 24 || canonical.authority?.apkSignatureVerify !== 'PASS' || canonical.authority?.apkAapt2Parse !== 'PASS') fail('current installability evidence incomplete');
...
console.log('CURRENT_CANONICAL_REFREEZE=0.2.0-alpha.2+2002');
