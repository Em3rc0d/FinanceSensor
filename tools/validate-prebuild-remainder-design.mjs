...
if (!failures.length) {
  ...
  const frozen = {
    schemaVersion: 'MK0_PREBUILD_REMAINDER_DESIGN_V1', base: 'ac195baebc2966521b2dcc73dfa3376ae09e6b4d',
    candidate: '0.2.0-alpha.2+2001', sourceCommit: 'f658363772b8d3652a81a8a4275a571f2f409ed8',
    apkSha256: '7fe14ac1ef62def124d1d15115809308a64e8d3cafffaa619b6c7105c40c8b9f', apkBytes: 182053563,
    package: 'com.financesensor.lab.gmailconnection.r2', scope: 'gmail.readonly',
    signerSha1: '63:2F:3A:4C:AE:C6:86:5B:C4:02:E8:82:12:2E:33:38:A6:EF:EB:D0'
  };
  const current = {
    candidate: '0.2.0-alpha.2+2008', sourceCommit: '45b605d29fe0b90f528e4f0f952ab878080b2f0b',
    apkSha256: 'eb4afc91357204419b3693efa973ba5bbcbd09a8037c3932269cea25363e7238', apkBytes: 182515867,
    signedApkSha256: 'a6e9e9441842f9de78147d8bef0103c63c1ad5b499963303111dbb99dfcd5277', signedApkBytes: 182538790,
    r1Receipt: 'graph/physical-receipts/ALPHA2-R1-TRUSTED-EDGE-SIGNING-2008-2026-09-14.json'
  };

  if (design.schemaVersion !== frozen.schemaVersion || design.project !== 'FinanceSensor' || design.mk !== 'MK0' || design.designFreeze !== 'PASS') fail('prebuild design identity/freeze mismatch');
  ...
  for (const [key,value] of Object.entries({candidate:frozen.candidate,sourceCommit:frozen.sourceCommit,apkSha256:frozen.apkSha256,apkBytes:frozen.apkBytes,package:frozen.package,scope:frozen.scope,stableSignerSha1:frozen.signerSha1})) if (snapshot[key] !== value) fail(`historical canonical Alpha.2 snapshot ${key} drifted`);

  const laws = new Set(design.executionLaws ?? []);
  for (const law of ['DESIGN_FREEZE_PASS_DOES_NOT_EQUAL_BUILD_READY','UNMAPPED_PRODUCT_BUILD_IS_FORBIDDEN','STATIC_PASS_DOES_NOT_EQUAL_PHYSICAL_PASS','APK_BUILD_PASS_DOES_NOT_EQUAL_BUILD_READY','RAW_TRUSTED_EDGE_EVIDENCE_DOES_NOT_ENTER_GITHUB','ONLY_SANITIZED_RECEIPTS_MAY_ENTER_GITHUB','SOURCE_OR_APK_IDENTITY_CHANGE_REOPENS_SIGNING_AND_OWNED_DEVICE_CAMPAIGN','BUILD_READY_TRUE_REQUIRES_G_MK0_CLOSED']) if (!laws.has(law)) fail(`missing execution law ${law}`);
  ...
  if (canonical.candidate !== current.candidate || canonical.sourceCommit !== current.sourceCommit || canonical.authority?.apkSha256 !== current.apkSha256 || canonical.authority?.apkBytes !== current.apkBytes) fail('current canonical +2008 identity mismatch');
  ...
  if (canonical.signing?.trustedEdgeSigningPass !== true || canonical.signing?.signedApkSha256 !== current.signedApkSha256 || canonical.signing?.signedApkBytes !== current.signedApkBytes || canonical.signing?.receipt !== current.r1Receipt) fail('current +2008 stable signing PASS drifted');
  ...
  if (canonical.physicalInstallabilityObservation?.inheritanceFromPriorCandidateAllowed !== false || canonical.physicalInstallabilityObservation?.status !== 'READY_FOR_OD0_CURRENT_CANDIDATE') fail('current +2008 physical frontier drifted');
  for (const id of ['0.2.0-alpha.2+2001','0.2.0-alpha.2+2002','0.2.0-alpha.2+2003','0.2.0-alpha.2+2004','0.2.0-alpha.2+2005','0.2.0-alpha.2+2006','0.2.0-alpha.2+2007']) if (!(canonical.nonAuthoritativeCandidates ?? []).some(x => x.candidate === id)) fail(`current canonical receipt must record ${id} supersession`);
  if (r1.status !== 'TRUSTED_EDGE_SIGNING_PASS' || r1.trustedEdgeSigningPass !== true || r1.signedApkSha256 !== current.signedApkSha256) fail('R1 +2008 PASS drifted');
  if (r2.status !== 'IN_PROGRESS' || r2.currentState?.r2PhysicalCampaign !== 'IN_PROGRESS' || r2.currentState?.nextGate !== 'OD0') fail('R2 must be open at OD0 after R1 PASS');
  ...
  if (byId.get('R0')?.status !== 'CLOSED' || byId.get('R1')?.status !== 'CLOSED' || byId.get('R2')?.status !== 'DESIGN_FROZEN_EXECUTION_OPEN') fail('R0/R1/R2 execution frontier drifted');
  ...
  const expectedDeps = new Map([['R1',['R0']],['R2',['R1']],['R3',['R2']],['R4',['R2']],['R5',['R2']],['R6',['R3','R4','R5']],['R7',['R6']],['R8',['R7']],['R9',['R8']],['R10',['R9']]]);
