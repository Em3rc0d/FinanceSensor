...
const globalForbidden = [
  [/\$\{\{\s*secrets\./, 'public CI may not reference repository/environment secrets'],
  [/runs-on:\s*\[[^\]]*self-hosted|runs-on:\s*self-hosted/i, 'public CI may not use persistent self-hosted runners'],
  [/FINANCESENSOR_GMAIL_ACCESS_TOKEN|FINANCESENSOR_GMAIL_REFRESH_TOKEN|FINANCESENSOR_GOOGLE_CLIENT_SECRET|FINANCESENSOR_GOOGLE_CREDENTIALS_PATH/,
    'public CI may not receive Gmail/OAuth credentials'],
  [/FINANCESENSOR_R2_LAB.*(PASSWORD|PRIVATE|KEYSTORE)|storePassword|keyPassword/i,
    'public CI may not receive stable private signing material'],
  [/REAL_OAUTH(?:_EXECUTED)?_BY_CI=(?:1|YES)|REAL_GMAIL(?:_EXECUTED)?_BY_CI=(?:1|YES)|REAL_FINANCIAL_DATA(?:_IN_CI)?=(?:1|YES)/i,
    'public CI may not claim real OAuth/Gmail/financial execution'],
  [/PHYSICAL_(?:ALPHA2|SQLCIPHER|PROFILE)_PASS=(?:1|YES)|BUILD_READY=(?:1|YES|true)|RELEASE_READY=(?:1|YES|true)/i,
    'public CI may not claim physical/build/release promotion'],
];
...
for (const file of ACTIVE_WORKFLOWS) {
  ...
  for (const [pattern, message] of globalForbidden) if (pattern.test(text)) fail(file, message);
}
...
for (const [file, contract] of Object.entries(contracts)) {
  ...
  for (const pattern of contract.forbidden ?? []) if (pattern.test(text)) fail(file, 'workflow executes a forbidden trusted-edge/provider surface');
