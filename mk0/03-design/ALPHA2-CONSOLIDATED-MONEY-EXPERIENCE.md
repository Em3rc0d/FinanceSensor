...
## Source role is not semantic exclusivity
...
Statements can also contain debits, fees, card payments, transfers, reversals and refunds. Gmail can contain credits or transfer notifications.
...
| Fact | Primary acquisition role | Secondary / reconciliation role | User-facing truth |
|---|---|---|---|
...
Public UX uses discrete states:
...
Transfers and card payments stay outside expense totals unless a separate economic effect proves otherwise.
...
4. `Flujo neto` only when the evidence contract permits it; otherwise explicitly `parcial`.
...
- `finanzas completas` while an expected source is missing;
- double-counted event + statement duplicates;
- treating transfers as expenses merely because money left an account.

## Privacy boundary

Raw Gmail content and raw statement documents remain on the authorized trusted edge unless a separately approved decision changes that boundary.

The web consumes canonical, minimized ledger records. Cloud synchronization remains governed by Q-003/Q-004/Q-005 and does not claim that E2EE automatically exempts FinanceSensor from Google restricted-scope requirements.

## Physical-test cadence

Physical APK tests are milestone gates, not development loops.

A new physical candidate is requested only after one exact source SHA passes:
